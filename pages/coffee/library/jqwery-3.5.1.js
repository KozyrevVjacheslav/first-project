/ *! *
Библиотека jQuery JavaScript v3 .5 .1
    *
    https: //jquery.com/
    *
    *
    ...
    *
    ...
    (функция(глобальная, фабрика) {
        ...
        // Передаем это, если окно еще не определено
            ...
            // arguments.callee.caller (trac-13335). Но с jQuery 3.0 (2016) строгий режим должен быть обычным
            ...
            var isFunction = function isFunction(obj) {
                ...
                // (то есть `typeof document.createElement (" object ") ===" function "`).
            ...
            // незащищенный в другом месте, кажется более безопасным определить глобальный только для этого модуля
            ...
            jQuery.extend = jQuery.fn.extend = function() {
                параметры
                var, имя, источник, копия, copyIsArray, клонировать,
                цель = аргументы[0] || {},
                    я = 1,
            ...
            jQuery.extend({
                ...
                expando: "jQuery" + (версия + Math.random()).replace(/ \ D /
                    g, ""),
                ...
                isPlainObject: function(obj) {
                    ...
                    // Объекты без прототипа (например, ʻObject.create (null) `) простые
                    if (!proto) {
                        вернуть истину;
                    }

                    // Объекты с прототипом являются простыми, если они были созданы глобальной функцией Object
                ...
                // push.apply (_, arraylike) бросает древний WebKit
            ...
            jQuery.each("Логическое число, строка, функция, массив, дата, регулярное выражение, объект ошибки, символ".split(""),
                function(_i, имя) {
            ...
            function isArrayLike(obj) {
                ...
                тип возвращаемого значения === "массив" || длина === 0 ||
                    typeof length === "число" && длина > 0 && (длина - 1) в obj;
            }
            var Sizzle =
                / *! *
...
"((?: \\\\. | [^ \\\\ () [\\]] |" + атрибуты + ") *) |" +
    ...
    matchExpr = {
        ...
        "PSEUDO": новый RegExp("^" + псевдоним),
        "РЕБЕНОК": новое регулярное выражение("^ :( только | первое | последнее | n-е | n-е-последнее) - (потомок | типа) (?: \\ (" +
...
rhtml = / HTML $ /
i,
rinputs = / ^ (?: input | select | textarea | button) $ /
i,
rheader = / ^ h \ d $ /
i,
...
rsibling = / [+ ~] /,
    ...
    runescape = new RegExp("\\\\ [\\ da-fA-F] {1,6}" + пробел + « ? | \\\\([ ^ \\r\\ n\\ f])», "грамм"),
    funescape = function(escape, nonHex) {
        ...
        вернуть nonHex ?
            ...
            // Заменить шестнадцатеричную escape-последовательность кодированной кодовой точкой Unicode
            // Поддержка: IE <= 11 +
    ...
    if (!seed) {
        ...
        if (documentIsHTML) {
            ...
            если(support.qsa &&
                ...
                // комбинаторы потомков, чего мы не хотим.
                // В таких случаях мы обходим поведение, добавляя префикс каждого селектора в
                // список с селектором ID, ссылающимся на контекст области.
                // Этот метод должен использоваться также, когда используется ведущий комбинатор
                // поскольку такие селекторы не распознаются querySelectorAll.
                // Спасибо Andrew Dupont за эту технику.
                если(nodeType === 1 &&
...
/ ** *
Создавайте кеши "ключ-значение"
ограниченного размера
    *
    @returns {
        function(string, object)
    }
Возвращает данные объекта после сохранения их на себе с помощью
    *
...
Поддержка тестирования с использованием элемента
    *
    @param { Function }
fn Передал созданный элемент и вернул логический результат
    *
    /
...
function assert(fn) {
    ...
    наконец - то {
        ...
        // По умолчанию удаляем из родительского
        if (el.parentNode) {
...
Добавляет один и тот же обработчик для всех указанных атрибутов
    *
    @param { String }
attrs Список атрибутов, разделенных конвейером
    *
    @param { Function }
обработчик Метод, который будет применяться
    *
    /
...
function createDisabledPseudo(disabled) {
    ...
    return function(elem) {
        ...
        if ("форма"
            ...
            // * перечисляет связанные с формой элементы в отключенном наборе полей
            ...
            if (elem.parentNode && elem.disabled === false) {
                ...
                // Элементы опций подчиняются родительской optgroup, если они есть
                if ("ярлык"
                    в элем) {
                    if ("ярлык"
                        в elem.parentNode) {
            ...
            // Некоторые жертвы попадают в нашу сеть (метка, легенда, меню, дорожка), но не должно
...
Обнаруживает узлы XML
    *
    @param { Element | Object }
elem Элемент или документ
    *
    @returns { Boolean }
Истина, если элемент elem не является узлом HTML XML
    *
    /
...
Expr.find["TAG"] = support.getElementsByTagName ?
    ...
    функция(тег, контекст) {
        ...
        // Отфильтровываем возможные комментарии
        if (tag === "*") {
...
if ((support.qsa = rnative.test(document.querySelectorAll))) {
    ...
    rbuggyQSA = rbuggyQSA.length && новый RegExp(rbuggyQSA.join("|"));
    rbuggyMatches = rbuggyMatches.length && новый RegExp(rbuggyMatches.join("|"));
	...
	function (a, b) {
		...
		// Идем вниз по дереву в поисках неточности
		while (ap [i] === bp [i]) {
			i ++;
		}
...
Sizzle.uniqueSort = function (results) {
	...
	// Если мы * не знаем * мы можем обнаружить дубликаты, предполагаем их присутствие
			...
			вернуть первый === 1 && последний === 0?
				...
				function (elem, _context, xml) {
					...
					if (parent) {
						...
						//: (только первая | последняя |) - (потомок | типа)
						if (simple) {
							while (dir) {
								узел = элемент;
								while ((node ​​= node [dir])) {
						...
						if (вперед && useCache) {
							...
							// Ищем ʻelem` из ранее кэшированного индекса
							...
							while ((node ​​= ++ nodeIndex && node && node [dir] ||
								...
								// Откат к поиску ʻelem` с самого начала
							...
							if (diff === false) {
								...
								// Используйте тот же цикл, что и выше, для поиска ʻelem` с самого начала
								while ((node ​​= ++ nodeIndex && node && node [dir] ||
									(diff = nodeIndex = 0) || start.pop ())) {
